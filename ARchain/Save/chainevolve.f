...
c         INCLUDE 'mergearc.inc'
         ...
         DO K = 1,3    ! needed to connect with common.
         cmet(K) = cmethod(K)
         END DO
*        tnext0=time+deltat    ! Seppo's original formulation.
*        wknx=tnext0/deltat
...
*        Skip merge routine (needs N-body interface).
         IF (icollision.lt.0) THEN ! handle a collison (suppressed by .lt.0)
...
*          IF(IWR.GT.0)WRITE(6,1232)time,(INAME(KW),KW=1,N)
           ...
           mmss=mmss+m(i)*m(j)
            ...
            if(step.eq.0.0) call Initial Stepsize(X,V,M,N,ee,step) ! New initial step
            stimex=step
            EPS=TOL
            Ncall=0
           END IF ! NEWREG
...
c        call Iterate2ExactTime(Y,Nvar,deltaT,f1,d1,f2,d2,x1,x2)
         end if
...
c        if(1.e-3*mmij.gt.m(i)*m(j).and.cmethod(2).ne.0.0)then 
     ...
     & CMXinc(3),CMVinc(3),ENERGYinc,Energrinc,CHTIMEinc,spin inc(3)
       ...
       subroutine COORDINATE DEPENDENT PERTURBATIONS(ACC) ! USER DEFINED (Not used!)
        INCLUDE 'ARCCOM2e2.CH'
...
C       are assumed to be in the vector ACC.
          ...
          TrueTIME=Taika+CHTIME ! this is (?) the time to be used for perturber coords
c                               ! chtime is measured from beginning of this CHAIN call
c           these times are in common
          
C---  init acc
        DO  I=1,3*N
         ki=i-(i-1)/3*3
          ACC(I)=0!-1.d-1*(x(i)+cmx(ki)) ! (test only) REMOVE this and REPLACE
        END DO
...
*       END DO ! I
        ...
        END
         ...
         subroutine Relativistic terms  ! replaced by PNPERT
...
c           pi= 3.14159265358979324d0
     ...
     &     +1./12.*(205+424*eta)*(m/r)*vr**2-.75*(113+2*eta)*vv*vr**2)
                   ...
                   B2p5=B2p5**2/(B2p5-B3p5/c**2) ! a test (maybe OK)
                   B3p5=0                        ! if above active, this is necessary
...
c        end do
       ...
       vv=cdot(v,v)
       er=cdot(e,x)
       RQ2=(-1+3*(er/r)**2)/(2*r**3) ! the quadrupole potential (exept factor Q2)
       U2b=m/r
       oc=1/c 
       do k=1,3
...
*          call Coordinate Dependent Perturbations (acc)  ! replaced 13/3/11.
           call XTPERT(ACC)
                 do i=1,n-1
                 do k=1,3
                 L=3*(i-1)
