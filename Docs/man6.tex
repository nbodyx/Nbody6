...
This code is mainly intended for laptops and workstations with {\tt UNIX}
operating system and GPUs.
A parallel implementation called {\ZZ{NBODY6\plusplus}} for PC
clusters and supercomputers is also available [Wang \etal 2015].
...
method [Makino \& Aarseth 1992] (for the parallel version see Aarseth 1999).
...
while interactions of compact subsystems are calculated by the chain
...
criterion [Mardling \& Aarseth 1999, Mardling 2008, Valtonen 2014] are reduced
to two-body configurations (so-called `mergers' as opposed to collisions).
All the relevant equations of motion are derived and discussed extensively in
a book [Aarseth 2003], together with algorithms which may be helpful
...
Note that the private {\ZZ{GRAPE6}} code {\ZZ{NBODY4}}
contains several important astrophysical processes which were included
...
tar file.\footnote{\tt ftp.ast.cam.ac.uk/pub/sverre.}
...
are extracted by `{\tt tar xvf nbody6.tar}' and copied to eight directories:
{\tt Ncode, Chain, Nchain, Docs, Block, ARchain, ARint, GPU2}.
...
The size of most large {\tt common} arrays is given by the parameter file
...
interface directories {\tt Nchain, ARint, Block, GPU2} by a soft link definition.
Once these parameters have been specified (dir {\tt Ncode}), the correct size of
the {\tt common} block is calculated automatically in routine {\tt mydump.f}.
The original modest choice of parameters ($N_{\rm max} = 4010$) produces a
...
It is also a good idea to save the new source files before making changes.
...
Table~\ref{options} or routine {\tt define.f}.
...
For most purposes, and a wide range of particle numbers, the original choice
of the accuracy parameters $\etaI, \etaR$ and $\etaU$ may suffice.
...
included in the directory {\tt Docs}.
Also see {\tt mcluster.c}.
A package for producing initially rotating models is also available
[Spurzem \& Einsel 2002, private communication].

The specially generated initial condition is read from {\tt fort.10} with
...
those with $T_0 + \Delta t_j = t_{\rm min}$, where $t_{\rm min}$ is the
smallest value of $t_{\rm new} = T_0 + \Delta t_j$ in the previous block.
...
examined at small quantized intervals $\le 10^2$~yr.
...
Typically $d < 100 a (1 + e)$ for equal masses.
...
Also note the text file {\tt fort.x} available on the website and in dir
{\tt Docs}.
...
The maximum eccentricity in a Kozai cycle is given by $e_{\rm max}$, while
...
escapers removed.
The accumulated energy error, {\tt DETOT}, gives the systematic drift
in total energy.
...
and are reset to zero above $2 \times 10^9$ (\cf {\tt NIRECT, NURECT, NRRECT}).
...
The KS method first appeared in a standard $N$-body code at the
Cambridge IAU Colloquium \#~10 in 1970 [Aarseth 1972].
...
[Mikkola \& Aarseth 1998]; or alternatively the KS block-step scheme
[Nitadori 2012].
Although the underlying mathematics is very precise, implementations are
something of a black art, with a variety of heuristic procedures.
This is particularly the case when dealing with interactions in compact
subsystems.
...
adopt the \cm approximation and introduce the KS solution for the outer
...
the chaos approach provided some theoretical justification and led to
a semi-analytical criterion [Mardling \& Aarseth 1999].
More recently, a general three-body stability criterion has been developed
from first principles [Mardling 2008].
It is valid for different masses and inclinations and was found to be more
reliable than older criteria.
The code also includes an optional averaging method
to model the eccentricity oscillations (Kozai cycles) induced by high
inclinations.
However, a new stability criterion [Valtonen 2014] has now been extensively
tested and appears more robust, particularly for large mass ratios.
This algorithm has now been adopted throughout.
...
For simplicity, these formulations do not include external perturbations.
...
Over the last few years, special efforts have gone into implementation of
algorithmic regularization [Mikkola \& Merritt 2009]. There are two new
directories, {\tt ARchain \& ARint} which are analogous to standard chain
regularization.
However, a full post-Newtonian treatment is also included [Aarseth 2012],
together with tidal disruption.
The different outcomes involve rather complicated decision-making.
Thus there are two channels for stellar disruption: either via a KS
regularization or inside a chain (ARC only).
Additionally, a new procedure for initiating chain or ARC regularization
for strongly perturbed KS binaries has been introduced which brings further
complications.
...
names.
It is then a simple matter to recover the relevant global locations of
ghosts for initialization.
...
A more comprehensive treatment of wind mass loss has also been included
in the 2015 version, together with a revised scheme for velocity kicks
and final masses of NSs/BHs.

For convenience, mass loss corrections are implemented when significant.
...
stars.
However, there is now (Nov 2015) a choice of modest kick velocities.
An optional procedure (\#~37) is included in order to avoid a sudden close
approach by high-velocity particles.

The code contains a realistic recipe for physical collisions,
...
variables rather than directly from the coordinates and velocities.
...
Hence a more detailed exploration of ejected stars should be restricted to
modest distances.
...
Note that eccentric orbits in the galactic plane can also be studied in this
model.
...
This facility can be particularly useful for cluster orbits in a 3D
...
treatment.
This ensures that the data is available with the highest accuracy at main
output times.
...
$R_{\rm cl} \propto 1/N$ while the interparticle separation scales as
$1/N^{1/3}$.
...
whereupon a {\tt common} save may occur on {\tt fort.1} (if \#~1~$>$ 0),
...
focusing.
Even if not present initially, such binaries may shrink via tidal
circularization or common envelope interactions.
In any case, some shrinkage by dynamical means is possible before ejection
by recoil intervenes.
Thus a small change in the total energy would give rise to a
...
\item Rosemary Mardling~~~~~~~~~~~~~~Tidal circularization
\item Keigo Nitadori~~~~~~~~~~~~~~~~~~~~GPU \& KS block-steps
\end{itemize}

\bigskip
Version 8.3.0 11/2015
...
\medskip
\noindent
Nitadori, K. [2012], `An implementation of KS regularization with Hermite
integrator and block timestep', unpublished

\medskip
\noindent
Nitadori, K. \& Aarseth, S.J. [2012], `Accelerating NBODY6 with graphics
processing units', \MN {\bf 424}, 545--52 
...
\medskip
\noindent
Valtonen, M. [2014], `Hiararchical stability criterion', AAS meeting 

\medskip
\noindent
Wang, L., \etal [2015], `The gravitational million-body problem',
 MNRAS {\bf 450}, 4070
...
$\gamma_{\rm max}$ &{\ZZ{GMAX}} &Termination criterion for soft binaries &0&001 \\
\hline\hline
\end {tabular}
\end{table}
...
the $N$-body directories (or just 'KZ' in the directory {\tt Chain} or
{\tt ARchain}).
...
%4 &Black hole mass histogram (>1; M/4 + 1) \\
...
31 &Cluster centre of mass correction \\
...
%41 &Pre-mainsequence stellar evolution (solar Z) \\
%43 &Velocity kick after GR binary coalescence \\
\hline\hline
\end{tabular}
\end{table}
