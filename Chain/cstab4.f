...
*
*
*       Sort particle separations (I3 & I4 form closest pair).
      CALL R2SORT(IJ,R2)
*
...
*     IF (1.0/RINV(2).LT.0.1*RSUM) THEN
*         CALL CSTAB3(ITERM)
*         GO TO 40
*     END IF
*
...
*       Note outer elements are good for pure QUAD (B-B configuration).
      RB2 = SQRT(R2(I3,I4))
      SEMI2 = 2.0/RB2 - VREL34/MB2
      SEMI2 = 1.0/SEMI2
*
*       Obtain the inclination of outer binary c.m.
...
*       Include small correction factor.
          PCRIT = QST*SEMI*(1.0 + 0.1*(SEMI2/SEMI))
      ELSE
          ITERM = 1
          GO TO 40
      END IF
...
*       Check hierarchical stability condition for bound close pair.
      ITERM = 0
      IF (PMIN.GT.PCRIT.AND.RB.GT.SEMI) THEN
          ITERM = -1
   ...
   20     FORMAT (' QUAD HIARCH   E =',F6.3,'  E1 =',F6.3,
     ...
     &                     '  PC =',E9.2,'  IN =',0P,F6.1)
          ...
          WRITE (82,30)  TIMEC, RI, NAMEC(I3), ECC, ECC1, SEMI, SEMI1,
     &                   PCRIT/PMIN, ALPHA
